...
const SIMPLIFIED_TO_TRADITIONAL_MAP: Record<string, string> = {
  ...
  '亚': '亞', '产': '產', '亩': '畝', '亲': '親', '仅': '僅', '从': '從', 
  '众': '眾', '仑': '侖', '仓': '倉', '仪': '儀', '们': '們', '优': '優', 
  '价': '價', '伤': '傷', '伥': '倀', '伪': '偽', '传': '傳', '伫': '佇', 
  '伟': '偉', '体': '體', '余': '餘', '佣': '傭', '佥': '僉', '侠侣': '俠侶', 
  '侥': '僥', '侦': '偵', '侧': '側', '侨': '僑', '侩': '儈', '偿': '償', 
  '俪': '儷', '俭': '儉', '债': '債', '倾': '傾', '储': '儲', '偾': '債', 
  '儿': '兒', '兀': '兀', '允': '允', '元': '元', '兄': '兄', '充': '充', 
  '兆': '兆', '先': '先', '光': '光', '克': '克', '免': '免', '兑': '兌', 
  '兔': '兔', '兎': '兔', '兖': '兗', '像': '像', '傻': '傻', '傌': '罵', 
  '兢': '兢', '党': '黨', '兰': '蘭', '关': '關', '兴': '興', '兹': '茲', 
  '养': '養', '兽': '獸', '单': '單', '卖': '賣', '门': '門', '闪': '閃', 
  '问': '問', '闯': '闖', '闰': '閏', '闲': '閑', '间': '間', '闵': '閔', 
  '闷': '悶', '闸': '閘', '闹': '鬧', '闺': '閨', '闻': '聞', '闼': '闥', 
  '闽': '閩', '闾': '閭', '闬': '閈', '阀': '閥', '阁': '閣', '阂': '閡', 
  '阃': '閫', '阄': '鬮', '阅': '閱', '阆': '閬', '阐': '闡', '阏': '閼', 
  '阍': '閽', '阌': '閿', '阋': '鬩', '阊': '閶', '阉': '閹', '闶': '閌',
  // 添加更多常用字的映射...
};
...
// 检查浏览器是否支持Intl.ChineseTonalSpelling
  // @ts-expect-error - 这个API可能不是所有浏览器都支持，但作为备选方案
  if (typeof Intl !== 'undefined' && Intl.ChineseTonalSpelling) {
    try {
      // @ts-expect-error - Intl.ChineseTonalSpelling may not be available in all browsers
